...
public class TimeStampManager {
	...
	*/
		...
		while (iter.hasNext()) {
			...
			if (cert_serial_number != null) {
				if (cert.getSerialNumber().equals (cert_serial_number)) {
					log.info("using certificate with serial: "+cert.getSerialNumber());
					certificate = cert;
				}
			} else {
				if (certificate == null) {
     ...
     * conforme �  la norme si ce n'est que la date n'est pas probante.
     ...
     */
    public static byte[] getTimestamp(byte[] empreinte, String idJeton, KeyStoreInfo ksInfo) throws Exception {
        try {
            KeyTools kt = new KeyTools();
            
	    KeyStore ks = kt.loadKeyStore(ksInfo.getPath(), ksInfo.getStoreFormat(), ksInfo
		    .getPassword());
        	
        	
            // recup keystore, certifs et clefs
            X509Certificate cert = (X509Certificate) ks.getCertificate(CERTIFICAT_NAME);
            PrivateKey pk = (PrivateKey) ks.getKey(CERTIFICAT_NAME, CERTIFICAT_PASSWORD.toCharArray());
            ...
            TimeStampTokenGenerator tokenGen = new TimeStampTokenGenerator(pk, cert, algorithme, TSA_POLICY_ID);
            tokenGen.setCertificatesAndCRLs(certs);

            TimeStampRequestGenerator gen = new TimeStampRequestGenerator();
            gen.setCertReq(true);

            TimeStampRequest req = gen.generate(algorithme, empreinte, new BigInteger(80, SecureRandom.getInstance("SHA1PRNG")));
            TimeStampToken token = tokenGen.generate(req, new BigInteger(idJeton), new Date(), "BC");
